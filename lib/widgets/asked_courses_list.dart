  ...
  Future<List> getListAskedCourses() async {
    ...
    for (int i = 0; i < listOfCourses.length; i++) {
      ...
      if (createdAt != null) {
        ...
        var courseCard = CoursesCard(listOfCourses[i]["title"], listOfCourses[i]["description"], listOfCourses[i]["date"], listOfCourses[i]["hour"], listOfCourses[i]["duration"], listOfCourses[i]["discipline"], listOfCourses[i]["place"], formatedDate);
        items.add(courseCard);
      } else {
        var courseCard = CoursesCard(listOfCourses[i]["title"], listOfCourses[i]["description"], listOfCourses[i]["date"], listOfCourses[i]["hour"], listOfCourses[i]["duration"], listOfCourses[i]["discipline"], listOfCourses[i]["place"], "null");
        items.add(courseCard);
      }
    }
    return items;
